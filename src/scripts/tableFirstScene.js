...
class FirstTableScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio("fxPegar", "assets/gameAssets/Audio/sounds/soundEffects/Fase1/fxPegarPolaroid.mp3");
    this.load.audio("fxSoltar", "assets/gameAssets/Audio/sounds/soundEffects/Fase1/fxSoltarPolaroid.mp3");
    this.load.audio("fxAcerto", "assets/gameAssets/Audio/sounds/soundEffects/Reutilizaveis/fxCorreto.mp3");
    this.load.audio("fxErro", "assets/gameAssets/Audio/sounds/soundEffects/Reutilizaveis/fxErrado.mp3");
  ...
  create() {
    ...
    // Efeitos sonoros
    this.fxPegar = this.sound.add("fxPegar");
    this.fxSoltar = this.sound.add("fxSoltar");
    this.fxAcerto = this.sound.add("fxAcerto");
    this.fxErro = this.sound.add("fxErro");
  ...
  for (let i = 0; i < polaroids.length; i++) {
    ...
    polaroids[i] = scene.add
      ...
      .on("pointerdown", function () {
        ...
        scene.fxPegar.play(); // Efeito de áudio de pegar polaroid
        if (clickDelay < 350) {
...
function addInput(scene) {
  scene.input.on("drag", (pointer, gameObject, dragX, dragY) => {
    ...
    scene.fxSoltar.play(); // Efeito de áudio de soltar polaroid
    depth += 1;
  });
  scene.input.on("drop", (pointer, gameObject, dropZone) => {
    for (let i = 0; i < zones.length; i++) {
      if (
        ...
        polaroidsFixed += 1;
        scene.fxAcerto.play(); // Efeito de áudio de acerto
        if (polaroidsFixed == 8) {
          nextButton(scene);
        }
        break;
        ...
        if (i === 7) {
          ...
          scene.fxErro.play(); // Efeito de áudio de erro
        }
      }
    }
  });
